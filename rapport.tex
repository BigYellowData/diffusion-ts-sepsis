...
\begin{enumerate}[leftmargin=2em, itemsep=0.2em]
    ...
    $\times 12.8$ plus élevée sur les prédictions incorrectes, Pearson $r = 0.43$)
    permet d'identifier en aval les cas où le modèle hésite et qui nécessitent
...
TimeGAN semi-sup.~\cite{yoon2019}   & 0.8363 & 0.0993 & \textbf{0.1269} & $-0.229$ & 0.014 \\
...
Variance MC --- prédictions correctes   & $5.34 \times 10^{-5}$ \\
Variance MC --- prédictions incorrectes & $6.85 \times 10^{-4}$ \\
...
\begin{subfigure}[b]{0.42\textwidth}
    ...
    \caption{Variance MC Dropout : erreurs vs corrects (ratio = $\times$12.8)}
\end{subfigure}
...
incorrectes est 12.8$\times$ supérieure à celle des prédictions correctes,
...
\midrule
5\%  & 108  & 0.8642 & 0.1443 & \textbf{0.1410} & 0.0170 \\
10\% & 215  & 0.8679 & 0.1363 & 0.1139 & 0.0088 \\
25\% & 539  & 0.8669 & \textbf{0.1484} & 0.1153 & 0.0179 \\
\rowcolor{bestrow}
50\% & 1078 & \textbf{0.8684} & 0.1356 & 0.1228 & \textbf{0.0058} \\
\bottomrule
\end{tabular}
\end{table}
...
un classifieur robuste avec seulement 215 fenêtres positives labellées.
...
en milieu clinique. La corrélation incertitude--erreur de \textbf{0.426} confirme
l'utilité du MC Dropout : la variance sur les prédictions incorrectes (au seuil
opérationnel de Youden) est \textbf{12.8 fois plus élevée} que sur les correctes
($6.85\times10^{-4}$ vs $5.34\times10^{-5}$).
...
\textbf{108 fenêtres positives labellées (5\%)}, le système atteint déjà AUROC = 0.864.

L'AUPRC oscille entre 0.136 et 0.148 sans tendance monotone (5\% : 0.144, 10\% : 0.136,
25\% : 0.148, 50\% : 0.136), avec le maximum atteint à 25\%. Le F1 culmine quant à lui
à 5\% (0.141), pas au ratio le plus riche en labels. Cette absence de monotonie est
\textbf{cohérente avec notre thèse centrale} : si la performance dépendait fortement
du nombre de labels, on observerait une croissance régulière entre 5\% et 50\%. Au
lieu de cela, la variation est faible et bruitée, ce qui confirme que la diffusion
apprend l'essentiel de sa structure sur les 94 000 fenêtres non labellées, et que le
guidage conditionnel n'apporte qu'un effet secondaire.

L'ECE varie aussi de façon non monotone (0.006 à 0.018 selon le ratio) mais reste
partout inférieure à 0.02, ce qui demeure cliniquement acceptable. Les fluctuations
ECE peuvent provenir de la stochasticité du training (un seul seed par ratio dans
notre étude).
...
\begin{enumerate}
    ...
    atteint AUROC = 0.868 et AUPRC = 0.136 sur le test set, surpassant cinq baselines
    dont l'approche signature gagnante du PhysioNet Challenge 2019.
    ...
    \textbf{+7.9 pts AUROC} vs Transformer vanilla (preuve directe de l'apport
    de l'augmentation) et \textbf{+2.7 pts} vs BiLSTM supervisé. À protocole
    identique, la diffusion surpasse également l'approche GAN (0.868 vs 0.836
    pour TimeGAN), ce qui justifie le choix architectural fait à la suite de
    Hirnschall.

    \item \textbf{La quantification d'incertitude par MC Dropout est exploitable.}
    La variance des 50 passes stochastiques est \textbf{×12.8 plus élevée} sur les
    prédictions incorrectes que sur les correctes (Pearson $r = 0.43$), avec un
    ECE de \textbf{0.009} qui place le modèle parmi les mieux calibrés de la
    comparaison. Le cas qualitatif \emph{FP incertain} (table~\ref{tab:cases})
